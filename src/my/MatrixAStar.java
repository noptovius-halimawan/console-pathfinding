...
public class MatrixAStar {
	...
	class Edge{
		...
		Edge(Node src, Node dst, int currDistance, int currHeuristics){
			this.src = src;
			this.dst = dst;
			this.currDistance = currDistance;
			this.currHeuristics = currHeuristics;
		}
	}
	...
	final int heuristicList[] = {20, 18, 13, 8, 0, 5, 9, 15, 14};
	...
	private void sortEdgesAscending() {
		Collections.sort(queue, new Comparator<Edge>() {
			...
			public int compare(Edge a, Edge b) {
				return (a.currDistance + a.currHeuristics) - (b.currDistance + b.currHeuristics);
			}
	...
	void addNeighborFromNode(Node curr) {
		for(int next = 0; next < num_node; next++) {
			...
			int nextHeuristic = heuristicList[next];

			if( nextCost == 0 || graph[next].visited == true) {
				continue;
			}			

			queue.add(new Edge(graph[curr.no], graph[next], curr.distance + nextCost, nextHeuristic));
		}
	}
